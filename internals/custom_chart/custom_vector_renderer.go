...
func CustomSVG(width, height int) (CustomRenderer, error) {
...
func CustomSVGWithCSS(css string, nonce string) func(width, height int) (CustomRenderer, error) {
	return func(width, height int) (CustomRenderer, error) {
		buffer := bytes.NewBuffer([]byte{})
		canvas := newCanvas(buffer)
		canvas.css = css
		canvas.nonce = nonce
...
func (vr *vectorRenderer) ResetStyle() {
	vr.s = &chart.Style{Font: vr.s.Font}
	vr.fc = nil
}
...
func (vr *vectorRenderer) ArcTo(cx, cy int, rx, ry, startAngle, delta float64) {
	startAngle = chart.RadianAdd(startAngle, math.Pi/2)
	endAngle := chart.RadianAdd(startAngle, delta)
	...
	dd := chart.RadiansToDegrees(delta)
...
func (vr *vectorRenderer) drawPath(s chart.Style) {
...
func (vr *vectorRenderer) MeasureText(body string) (box chart.Box) {
	if vr.s.GetFont() != nil {
		vr.fc = &font.Drawer{
		...
		box = box.Corners().Rotate(chart.RadiansToDegrees(*vr.c.textTheta)).Box()
	}
	return
}
...
func (c *canvas) Path(d string, style chart.Style) {
...
func (c *canvas) Text(x, y int, body string, style chart.Style) {
	if c.textTheta == nil {
		c.w.Write([]byte(fmt.Sprintf(`<text x="%d" y="%d" %s>%s</text>`, x, y, c.styleAsSVG(style), body)))
	} else {
		transform := fmt.Sprintf(` transform="rotate(%0.2f,%d,%d)"`, chart.RadiansToDegrees(*c.textTheta), x, y)
		c.w.Write([]byte(fmt.Sprintf(`<text x="%d" y="%d" %s%s>%s</text>`, x, y, c.styleAsSVG(style), transform, body)))
	}
}

func (c *canvas) Circle(x, y, r int, style chart.Style) {
...
func (c *canvas) getStrokeDashArray(s chart.Style) string {
...
func (c *canvas) getFontFace(s chart.Style) string {
	family := "sans-serif"
	if s.GetFont() != nil {
...
func (c *canvas) styleAsSVG(s chart.Style) string {
	sw := s.StrokeWidth
	sc := s.StrokeColor
	fc := s.FillColor
	fs := s.FontSize
